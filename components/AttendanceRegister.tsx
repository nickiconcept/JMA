...
import { Student, Attendance, ClassDefinition, Term } from '../types';
...
const AttendanceRegister: React.FC<Props> = ({ students, attendanceRecords, currentClass, onSaveAttendance }) => {
  ...
  React.useEffect(() => {
    const existing = attendanceRecords.filter(a => a.date === selectedDate && a.classId === currentClass.id);
  ...
  const handleSave = () => {
    const newRecords: Attendance[] = Object.keys(draftRecords).map(studentId => ({
      ...
      term: Term.FIRST, // Should use constant
      session: '2024/2025' // Should use constant
    }));
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-md">
      ...
      <div className="flex space-x-2 mb-4">
          <button onClick={() => markAll('PRESENT')} className="text-xs bg-green-100 text-green-700 px-3 py-1 rounded">Mark All Present</button>
          <button onClick={() => markAll('ABSENT')} className="text-xs bg-red-100 text-red-700 px-3 py-1 rounded">Mark All Absent</button>
      </div>
